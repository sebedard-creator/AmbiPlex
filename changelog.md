...
- **Architecture**: Intégration d'un système de sous-titres visuels .wledsub.lz4 généré en amont par FFmpeg pour supprimer l'utilisation CPU sur le Raspberry Pi / serveur local.
- **wled_reader.py**: Création du module de décompression JIT via l'algorithme lz4 et mappage direct en RAM avec 
umpy.memmap.
- **web.py**: Détection automatique des fichiers compatibles. Mode "Zéro CPU" activé (court-circuitage total de l'instance MPV).
- **led_engine.py**: Refactorisation pour accepter les tableaux de couleurs RGB565 pré-calculés, en conservant l'application de la luminosité et du lissage en temps réel.
- **Interface UI**: Affichage dynamique (SSE) de l'activation du mode (WLEDSUB) au lieu du mode MPV.
- **Dépendances**: Ajout de lz4 dans le equirements.txt.
